...
| 3.13 | `svs_utils.py:250` installing a fresh `nn.Embedding` is the root cause of the 151M-unsaved-params bug | Direct code read | **SAFE TO ASSERT** |
| 3.14 | `score_lm_head` has the same unsaved-parameter problem | Direct code read, lines 115–130 | **SAFE TO ASSERT** |
